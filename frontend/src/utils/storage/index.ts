...
export class Storage extends StorageBase {
  ...
  // Sensitive values — Keychain (iOS) / EncryptedSharedPreferences (Android) con fallback resiliente.
  ...
  async secureGet<Fallback extends StorageItemValue>(
    ...
    // Fallback ad AsyncStorage per massima resilienza
    try {
      const fallbackRaw = await AsyncStorage.getItem(`@sec_${key}`);
      return this.retrieve(fallbackRaw, fallback);
    } catch (e) {
      this.warn("secureGet:fallback", key, e);
      return fallback;
    }
  }
  ...
  async secureSet<Value extends StorageItemValue>(
    ...
    // Salva sempre anche la copia fallback in AsyncStorage
    try {
      await AsyncStorage.setItem(`@sec_${key}`, JSON.stringify(value));
      return true;
    } catch (e) {
      this.warn("secureSet:fallback", key, e);
      return secureStoreOk;
    }
  }
  ...
  async secureRemove(key: string): Promise<boolean> {
    ...
    try {
      ...
      this.warn("secureRemove:fallback", key, e);
    }
    return true;
  }
}
