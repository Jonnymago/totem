...
function handleStaticFile(socket: any, rawPath: string) {
  try {
    ...
    // Remove any trailing slash for lookup, except root '/'
    if (cleanPath.length > 1 && cleanPath.endsWith('/')) {
      cleanPath = cleanPath.slice(0, -1);
    }

    // Direct alias mappings for remote admin panel & app routes
    if (
      cleanPath === '/remote' ||
    ...
    // Strip prefix if web app asset is requested with /remote/ prefix
    ...
    // Fallback: if not found by direct key
    if (!file) {
