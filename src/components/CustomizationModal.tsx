      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4 overflow-y-auto">
        ...
        >
          ...
          <div className="relative h-56 bg-zinc-800">
            ...
            <div className="absolute bottom-4 left-6 right-6">
              <span className="text-xs font-bold uppercase tracking-wider bg-rose-500/20 text-rose-400 border border-rose-500/30 px-3 py-1 rounded-full inline-block mb-2">
                {product.product_type === 'combo' ? t('products.combo_menu') : t('products.customize')}
              </span>
              <h2 className="text-3xl font-extrabold text-white">{translateCustomerMenuText(product.name, lang)}</h2>
              <p className="text-zinc-300 text-sm mt-1">{translateCustomerMenuText(product.description, lang)}</p>
            </div>
          </div>
              ...
              product.base_ingredients.length > 0 && (
                <div>
                  <h3 className="text-base font-bold text-white mb-3 flex items-center gap-2">
                    {t('products.without_ingredients')}
                    <span className="text-xs font-normal text-zinc-400">
                      {t('products.tap_to_remove')}
                    </span>
                  </h3>
                    ...
                    {product.base_ingredients.map((ing) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {translateCustomerMenuText(ing, lang)}
                        </button>
                      );
                    })}
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {product.extra_additions.map((extra) => {
                      ...
                      return (
                        ...
                        >
                          <span className="font-medium text-sm">{translateCustomerMenuText(extra.name, lang)}</span>
                  ...
                  <div key={group.name} className="p-4 bg-zinc-800/40 rounded-2xl border border-zinc-800">
                    <div className="flex items-center justify-between mb-3">
                      <h3 className="text-base font-bold text-white flex items-center gap-2">
                        {translateCustomerMenuText(group.name, lang)}
                        {group.min_selection > 0 && (
                          <span className={`text-xs px-2 py-0.5 rounded-md ${isSatisfied ? 'bg-emerald-500/20 text-emerald-400' : 'bg-amber-500/20 text-amber-400'}`}>
                            {isSatisfied ? t('products.selected') : t('products.required')}
                          </span>
                        )}
                      </h3>
                      <span className="text-xs text-zinc-400">
                        {group.max_selection === 1 ? t('products.choose_one') : `${t('products.choose_up_to')} ${group.max_selection}`}
                      </span>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      {group.options.map((option) => {
                        ...
                        return (
                          ...
                          >
                            <span className="text-sm">{translateCustomerMenuText(option.name, lang)}</span>
          ...
          <div className="p-6 bg-zinc-950 border-t border-zinc-800 flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            >
              <span>{t('products.add_to_order')}</span>
