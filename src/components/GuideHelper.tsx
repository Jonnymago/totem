...
export const GuideHelper: React.FC<GuideHelperProps> = ({ embedded = false }) => {
  ...
  const filteredChapters = chapters.filter((ch) => {
    ...
    if (
      ...
      ch.id !== 'ch8_licensing' &&
      ch.id !== 'ch10_multi_totem_printers'
    ) {
      return false;
    }
    if (
              ...
              >
                ...
                {isExpanded && (
                  <div className="px-4 sm:px-6 pb-6 pt-2 border-t border-slate-100 bg-slate-50/50">
                    ...
                    {/* Interactive Topology Visual Diagram for Chapter 10 */}
                    {ch.id === 'ch10_multi_totem_printers' && (
                      <div className="my-5 p-5 bg-slate-900 text-white rounded-2xl border border-slate-800 space-y-4">
                        <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                          <div className="flex items-center gap-2">
                            <Network className="w-5 h-5 text-indigo-400" />
                            <span className="font-bold text-sm text-slate-200">
                              Schema Architetturale Master / Satellite & Routing Stampanti
                            </span>
                          </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
                          ...
                          <div className="bg-slate-800/80 p-3.5 rounded-xl border border-slate-700 space-y-2.5">
                            <span className="text-[11px] font-bold uppercase tracking-wider text-indigo-300 flex items-center gap-1">
                              <Server className="w-3.5 h-3.5" /> 1. Totem Touchscreen
                            </span>
                            <div className="bg-indigo-950/60 border border-indigo-500/40 p-2.5 rounded-lg">
                              <div className="font-bold text-white flex items-center justify-between">
                                <span>👑 Totem Master #1</span>
                                <span className="text-[9px] bg-indigo-500/30 text-indigo-200 px-1.5 py-0.5 rounded font-mono">
                                  Port 3000
                                </span>
                              </div>
                              <p className="text-[10px] text-slate-300 mt-1">
                                Gestisce Licenza Google Play, Database Menu e Numerazione progressiva unificata.
                              </p>
                            </div>
                            <div className="bg-slate-900/90 border border-slate-700 p-2.5 rounded-lg">
                              <div className="font-bold text-slate-200 flex items-center justify-between">
                                <span>🛰️ Totem Satellite #2..N</span>
                                <span className="text-[9px] bg-slate-700 text-slate-300 px-1.5 py-0.5 rounded font-mono">
                                  Sync LAN
                                </span>
                              </div>
                              <p className="text-[10px] text-slate-400 mt-1">
                                Eredita licenza e menu in tempo reale. Zero costi aggiuntivi di licenza.
                              </p>
                            </div>
                          </div>

                          {/* Network Sync Hub */}
                          <div className="bg-slate-800/80 p-3.5 rounded-xl border border-slate-700 space-y-2.5 flex flex-col justify-between">
                            <div>
                              <span className="text-[11px] font-bold uppercase tracking-wider text-emerald-400 flex items-center gap-1">
                                <Network className="w-3.5 h-3.5" /> 2. Rete Locale (LAN/Wi-Fi)
                              </span>
                              <div className="bg-emerald-950/40 border border-emerald-500/30 p-2.5 rounded-lg mt-2 space-y-1.5">
                                <div className="flex items-center gap-1.5 text-emerald-300 font-semibold text-[11px]">
                                  <CheckCircle2 className="w-3.5 h-3.5" /> Auto-Discovery mDNS/UDP
                                </div>
                                <p className="text-[10px] text-slate-300">
                                  Scansione automatica dei nodi Master attivi sulla sottorete con latenza &lt; 5ms.
                                </p>
                              </div>
                            </div>
                            <div className="p-2 bg-slate-950/70 rounded-lg text-center border border-slate-800">
                              <span className="text-[10px] font-mono text-emerald-400">
                                Continuità garantita senza Internet
                              </span>
                            </div>
                          </div>

                          {/* Printers & KDS Routing */}
                          <div className="bg-slate-800/80 p-3.5 rounded-xl border border-slate-700 space-y-2">
                            <span className="text-[11px] font-bold uppercase tracking-wider text-rose-400 flex items-center gap-1">
                              <Printer className="w-3.5 h-3.5" /> 3. Instradamento Reparti
                            </span>
                            <div className="space-y-1.5 text-[10px]">
                              <div className="p-1.5 bg-slate-900 rounded border border-slate-700 flex items-center justify-between">
                                <span className="font-semibold text-amber-300">🍳 Cucina Calda</span>
                                <span className="text-slate-400">Stampante 1 + KDS Cucina</span>
                              </div>
                              <div className="p-1.5 bg-slate-900 rounded border border-slate-700 flex items-center justify-between">
                                <span className="font-semibold text-rose-300">🍕 Forno Pizzeria</span>
                                <span className="text-slate-400">Stampante 2 + KDS Pizza</span>
                              </div>
                              <div className="p-1.5 bg-slate-900 rounded border border-slate-700 flex items-center justify-between">
                                <span className="font-semibold text-cyan-300">🍹 Bar & Bevande</span>
                                <span className="text-slate-400">Stampante 3 + KDS Bar</span>
                              </div>
                              <div className="p-1.5 bg-slate-900 rounded border border-slate-700 flex items-center justify-between">
                                <span className="font-semibold text-emerald-300">🧾 Cassa / Cliente</span>
                                <span className="text-slate-400">Scontrino Cortesia</span>
                              </div>
                            </div>
                          </div>
                        </div>
