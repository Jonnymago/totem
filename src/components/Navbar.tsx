      ...
      <header className="sticky top-0 z-40 bg-zinc-950/90 backdrop-blur-md border-b border-zinc-800/80 px-4 sm:px-8 py-3.5 flex items-center justify-between shadow-xl">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <span className="hidden md:inline">{t('common.kitchen')}</span>
          </button>
          ...
          {view !== 'welcome' && view !== 'order-confirmation' && view !== 'take-number' && (
            <button
              ...
              className="relative p-3.5 bg-rose-600 hover:bg-rose-500 text-white font-extrabold rounded-2xl shadow-lg shadow-rose-600/25 flex items-center gap-2.5 transition-all active:scale-95"
            >
              <ShoppingBag className="w-5 h-5" />
              <span className="text-sm">{t('common.cart')}</span>
              {cartItemCount > 0 && (
