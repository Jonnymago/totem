...
export const WelcomeView: React.FC = () => {
  ...
  // Reset momentary session language on welcome view mount
  useEffect(() => {
  ...
  return (
    <div className="relative min-h-screen bg-zinc-950 flex flex-col justify-between p-6 sm:p-12 overflow-hidden selection:bg-rose-500 selection:text-white">
      ...
      {/* Top Header Controls: Language Selector & Secret Admin Dot */}
      <div className="flex items-center justify-between z-20 w-full">
        {/* Language Selector for Customers */}
        <LanguageSelector mode="customer-session" />

        {/* Secret Admin Trigger */}
        <button
          onClick={handleSecretDotPress}
          className="relative w-9 h-9 rounded-full bg-zinc-800/60 border border-zinc-700/50 hover:bg-zinc-700 text-zinc-400 hover:text-white flex items-center justify-center transition-all opacity-80 hover:opacity-100"
      ...
      <div className="my-auto max-w-4xl mx-auto w-full text-center space-y-8 z-10 py-8">
        ...
        >
          {logo ? (
            ...
            <div className="w-28 h-28 rounded-3xl bg-rose-600 text-white flex items-center justify-center shadow-2xl shadow-rose-600/30 border border-rose-400/20">
          ...
          <div className="space-y-2">
            <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-zinc-900 border border-zinc-800 text-rose-400 text-xs font-bold uppercase tracking-widest">
              <Sparkles className="w-3.5 h-3.5" /> {t('welcome.kiosk_badge')}
            </span>
        ...
        >
          ...
          <button
            ...
            className="group relative p-8 bg-rose-600 hover:bg-rose-500 border border-rose-400/30 rounded-3xl text-left transition-all duration-300 shadow-xl shadow-rose-600/20 hover:shadow-rose-600/40 flex flex-col justify-between gap-8 active:scale-[0.98]"
          >
            <div className="flex items-center justify-between">
