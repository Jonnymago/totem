...
import { translateCustomerMenuText, useCustomerMenuGlossary } from '../utils/customerMenuTranslation';
...
export const ProductsView: React.FC = () => {
  ...
  const filteredProducts = products.filter((p) => {
    ...
    if (search.trim()) {
      ...
      const translatedName = translateCustomerMenuText(p.name, lang).toLowerCase();
      const translatedDesc = translateCustomerMenuText(p.description, lang).toLowerCase();
      return (
    ...
    <div className="min-h-[calc(100vh-70px)] bg-zinc-950 p-4 sm:p-8 max-w-7xl mx-auto">
      ...
      <div className="flex items-center gap-3 overflow-x-auto pb-4 custom-scrollbar mb-6">
        ...
        >
          ...
          <span>{t('products.all_categories')}</span>
        </button>
        ...
        >
          {t('products.all_dishes')} ({products.filter((p) => p.available).length})
        </button>
        ...
        {categories.map((cat) => {
          ...
          return (
            ...
            >
              {translateCustomerMenuText(cat.name, lang)}
            </button>
          );
        })}
      </div>
      ...
      <div className="mb-6 flex items-center justify-between">
        <h2 className="text-2xl font-black text-white">
          {activeCategory ? translateCustomerMenuText(activeCategory.name, lang) : t('products.all_dishes')}
        </h2>
        <span className="text-xs text-zinc-400 font-semibold">
          {filteredProducts.length} {t('products.options_available')}
        </span>
      </div>
      ...
      {filteredProducts.length === 0 ? (
        <div className="text-center py-16 bg-zinc-900/40 border border-zinc-800 rounded-3xl p-8">
          <p className="text-zinc-400 text-base font-semibold">{t('products.none_found')}</p>
          <p className="text-zinc-600 text-xs mt-1">
            {t('products.none_found_sub')}
          </p>
        </div>
      ) : (
            ...
            >
              ...
              <div className="relative h-48 bg-zinc-800 overflow-hidden">
                {product.image ? (
                  <img
                    src={product.image}
                    alt={translateCustomerMenuText(product.name, lang)}
              ...
              <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
                <div>
                  <h3 className="text-xl font-extrabold text-white group-hover:text-rose-400 transition-colors">
                    {translateCustomerMenuText(product.name, lang)}
                  </h3>
                  <p className="text-zinc-400 text-xs mt-1 line-clamp-2 leading-relaxed">
                    {translateCustomerMenuText(product.description, lang)}
                  </p>
