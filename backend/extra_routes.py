...
def register_extra_routes(api_router: APIRouter) -> None:
    ...
    async def sync_backup(backup: BackupData, username: str = Depends(verify_token)):
        ...
        if backup.translation_glossary is not None:
            await db.translation_glossary.delete_many({})
            if backup.translation_glossary:
                await db.translation_glossary.insert_one({
                    "entries": backup.translation_glossary,
                    "updated_at": datetime.now(UTC)
                })
        return {"message": "Backup synchronized successfully"}

    @api_router.get("/translation-glossary")
    @api_router.get("/admin/translation-glossary")
    async def get_translation_glossary():
        doc = await db.translation_glossary.find_one({})
        if not doc:
            return {}
        return doc.get("entries", {})

    @api_router.post("/translation-glossary")
    @api_router.post("/admin/translation-glossary")
    @api_router.put("/translation-glossary")
    @api_router.put("/admin/translation-glossary")
    async def save_translation_glossary(payload: dict):
        entries = payload.get("entries", payload) if isinstance(payload, dict) else {}
        doc = await db.translation_glossary.find_one({})
        current_entries = doc.get("entries", {}) if doc else {}
        for k, v in entries.items():
            if isinstance(v, dict):
                if k not in current_entries:
                    current_entries[k] = {}
                current_entries[k].update(v)
        await db.translation_glossary.update_one(
            {},
            {"$set": {"entries": current_entries, "updated_at": datetime.now(UTC)}},
            upsert=True,
        )
