...
async def admin_login(credentials: AdminLogin):
    ...
    try:
        valid = bcrypt.checkpw(credentials.password.encode("utf-8"), admin["password_hash"].encode("utf-8"))
...
async def admin_pin_login(credentials: PinLogin):
    settings = await db.settings.find_one({})
    expected_pin = settings.get("admin_pin") if settings else None
    entered_pin = (credentials.pin or "").strip()
    if not expected_pin or not entered_pin or entered_pin != expected_pin:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="PIN non valido")
    admin = await db.admin_users.find_one({})
    if not admin:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Admin non configurato")
    return Token(access_token=create_access_token(data={"sub": admin["username"]}))
...
async def get_settings():
    settings = await db.settings.find_one({})
    if not settings:
        default = {"restaurant_name": "TOTEM RISTORANTE", "logo": "", "auto_print_courtesy": True, "auto_print_kitchen": True, "order_reset_mode": "daily", "reset_time": "06:00", "admin_pin": None}
...
async def update_settings(settings_update: SettingsUpdate, username: str = Depends(verify_token)):
    ...
    if existing:
        ...
        base = {"restaurant_name": "TOTEM RISTORANTE", "logo": "", "auto_print_courtesy": True, "auto_print_kitchen": True, "order_reset_mode": "daily", "reset_time": "06:00", "last_reset_at": None, "admin_pin": None}
...
async def reset_order_number(username: str = Depends(verify_token)):
    ...
    if existing:
        ...
        await db.settings.insert_one({"restaurant_name": "TOTEM RISTORANTE", "logo": "", "auto_print_courtesy": True, "auto_print_kitchen": True, "order_reset_mode": "manual", "reset_time": "06:00", "last_reset_at": now, "admin_pin": None, "updated_at": now})
...
async def create_category(category: CategoryCreate, username: str = Depends(verify_token)):
    category_dict = category.dict(); category_dict["created_at"] = datetime.utcnow()
    result = await db.categories.insert_one(category_dict); category_dict["id"] = str(result.inserted_id)
...
async def create_product(product: ProductCreate, username: str = Depends(verify_token)):
    product_dict = product.dict(); product_dict["created_at"] = datetime.utcnow()
    result = await db.products.insert_one(product_dict); product_dict["id"] = str(result.inserted_id)
...
async def seed_database(force: bool = False):
    ...
    await db.settings.insert_one({"restaurant_name": "TOTEM RISTORANTE", "logo": "", "auto_print_courtesy": True, "auto_print_kitchen": True, "order_reset_mode": "daily", "reset_time": "06:00", "last_reset_at": None, "admin_pin": None, "updated_at": datetime.utcnow()})
    password = os.environ.get("INITIAL_ADMIN_PASSWORD")
    if not password:
        raise HTTPException(status_code=500, detail="INITIAL_ADMIN_PASSWORD is required for seed")
    password_hash = bcrypt.hashpw(password.encode('utf-8'), bcrypt.gensalt()).decode('utf-8')
    await db.admin_users.insert_one({"username": os.environ.get("INITIAL_ADMIN_USERNAME", "admin"), "password_hash": password_hash, "created_at": datetime.utcnow()})
    
    # Remaining seed data is intentionally unchanged below this point.
